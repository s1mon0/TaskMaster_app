...
export default function Sidebar({ lists, activeListId, onListClick, onCreateList, onDeleteList, onEditList, isDarkMode, toggleDarkMode, tasks, newListName, setNewListName }) {
  ...
  // Logika pro styl dlaždice (grafika + animace)
  const getTileClass = (id) => {
    ...
    if (isActive) {
      // Aktivní ostruvek s vypouklým stínem
      return `${baseClass} bg-white dark:bg-[#2c2c2e] shadow-lg border-transparent scale-[1.02]`;
    } else {
      // Hezké pozadí s jemným okrajem pro inaktivní dlaždice v light módu
  ...
  return (
    <div className={`w-full md:w-80 lg:w-[360px] bg-[#f2f2f7] dark:bg-[#000000] flex flex-col h-full ${activeListId ? 'hidden md:flex' : 'flex'} z-10 transition-colors duration-300`}>
      ...
      {/* Horní část - pouze s vypínačem theme (Logo ODSTRANĚNO) */}
      <div className="px-6 pt-10 pb-4 flex items-center justify-end"> {/* Sníženo pt, zarovnáno doprava */}
        <button onClick={toggleDarkMode} className="p-2.5 rounded-full bg-white dark:bg-[#1c1c1e] shadow-sm hover:scale-105 active:scale-95 transition-transform">
          {isDarkMode ? <Sun size={20} /> : <Moon size={20} />}
        </button>
      </div>

      <div className="flex-1 overflow-y-auto px-6 py-2 space-y-10 pb-10">
        {/* Apple Style Dlaždice (Vylepšená grafika + animace) */}
        <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div onClick={() => onListClick('my-day')} className={getTileClass('my-day')}>
              <div className="flex justify-between items-start mb-3">
                ...
                <span className={`text-3xl font-bold tracking-tight ${activeListId === 'my-day' ? 'text-[#ff9500]' : ''}`}>
                  {todayCount}
                </span>
              </div>
            ...
            <div onClick={() => onListClick('scheduled')} className={getTileClass('scheduled')}>
              <div className="flex justify-between items-start mb-3">
                ...
                <span className={`text-3xl font-bold tracking-tight ${activeListId === 'scheduled' ? 'text-[#007aff]' : ''}`}>
                  {scheduledCount}
                </span>
              </div>
            ...
            <div onClick={() => onListClick('all')} className={`${getTileClass('all')} col-span-2 p-3.5 flex-row justify-between items-center h-16`}>
                ...
                <span className={`text-2xl font-bold tracking-tight mr-1 ${activeListId === 'all' ? 'text-gray-900 dark:text-gray-100' : 'text-gray-600 dark:text-gray-400'}`}>
                  {allCount}
                </span>
            </div>
          </div>
        ...
        <div>
          ...
          <form onSubmit={onCreateList} className="mt-5 px-1 relative">
            <input 
              ...
              className="w-full bg-transparent py-3.5 pl-4 pr-16 outline-none focus:bg-white dark:focus:bg-[#1c1c1e] rounded-xl transition-all border border-transparent focus:border-gray-200 dark:focus:border-gray-800 focus:shadow-sm"
            />
            {newListName.trim() && (
